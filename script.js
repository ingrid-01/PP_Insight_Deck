...
const insights = [
  ...
  },
  {
    ...
    date: "Jan 2026",
    title: "Her (그녀)",
    content: "사랑은 사회적으로 용인된 미친 짓이다. 우리는 관계를 통해 서로를 성장시키지만, 때로는 그 성장이 이별을 부르기도 한다.",
    reflect: "AI와의 사랑을 다루지만 결국 인간 관계의 본질을 묻는다. 나는 관계 속에서 상대방을 내 방식대로 정의하려 하지 않았나?",
    action: null,
    discussionTopic: "기술이 발전하여 완벽한 정신적 교감이 가능한 AI가 나온다면, 육체적 사랑 없는 플라토닉 러브가 주류가 될 수 있을까?",
    dialogue: null,
  }
];
...
// 카테고리별 스타일 매핑
const styles = {
  news: { badgeBg: "bg-accent-news/10", badgeText: "text-accent-news", icon: "newspaper" },
  fiction: { badgeBg: "bg-accent-fiction/10", badgeText: "text-accent-fiction", icon: "auto_stories" },
  nonfiction: { badgeBg: "bg-accent-nonfiction/10", badgeText: "text-accent-nonfiction", icon: "menu_book" },
  movie: { badgeBg: "bg-accent-movie/10", badgeText: "text-accent-movie", icon: "movie" },
  art: { badgeBg: "bg-accent-art/10", badgeText: "text-accent-art", icon: "palette" },
  media: { badgeBg: "bg-accent-media/10", badgeText: "text-accent-media", icon: "play_circle" }
};
...
function updateFilterButtons() {
    const filters = ['all', 'nonfiction', 'news', 'fiction', 'movie', 'art', 'media'];
    filters.forEach(type => {
        const btn = document.getElementById(`filter-${type}`);
        if (!btn) return; 
        if (type === currentFilter) {
            btn.className = "px-3 py-1.5 rounded-full bg-primary text-white text-xs font-bold shadow-sm transition-all";
        } else {
            btn.className = "px-3 py-1.5 rounded-full bg-white border border-border text-text-sub text-xs font-bold transition-all hover:text-primary hover:bg-background-hover";
        }
    });
}
...
function renderInsights() {
  ...
  // 사이드바 통계 업데이트
  if(document.getElementById("stat-month")) document.getElementById("stat-month").innerText = insights.length; 
  if(document.getElementById("stat-hub")) document.getElementById("stat-hub").innerText = counts.logged; 
  if(document.getElementById("stat-total")) document.getElementById("stat-total").innerText = insights.length; 
...
form.addEventListener("submit", (e) => {
    ...
    let subCat = "기타";
    const catMap = { 
        news: "신문기사", fiction: "문학", nonfiction: "비문학", 
        movie: "영화", art: "전시/관람", media: "영상/미디어" 
    };
...
function openRichInputModal(type) {
    ...
    inputContainer.className = "flex flex-col gap-4 rounded-xl bg-white border border-[#d1d5db] p-1 focus-within:border-primary focus-within:ring-1 focus-within:ring-primary shadow-sm transition-all";
    ...
    if (type === 'reflect') {
        ...
        inputField.placeholder = "예: 이 문구는 내가 지난주에 겪었던 프로젝트 문제와 정확히 일치한다...";
        inputContainer.classList.add("bg-accent-dialogue/5", "border-accent-dialogue/20");
    } 
    else if (type === 'action') {
        titleEl.innerHTML = `<span class="material-symbols-outlined text-accent-action text-[24px]">bolt</span> 실천 과제 (Action)`;
        descEl.innerText = "이 통찰을 삶에 적용하기 위해 당장 실천할 수 있는 행동은 무엇인가요?";
        inputField.placeholder = "예: 내일 아침 30분 일찍 일어나서 글쓰기 시작하기";
        inputContainer.classList.add("bg-accent-action/5", "border-accent-action/20");
    } 
    else if (type === 'dialogue') {
        titleEl.innerHTML = `<span class="material-symbols-outlined text-primary text-[24px]">forum</span> 대화 로그 (Dialogue)`;
        descEl.innerText = "가족, 친구와 나눈 대화 중 기억하고 싶은 핵심 내용을 기록하세요.";
        ...
        descEl.innerText = "이 인사이트를 바탕으로 타인과 논의해보고 싶은 질문을 던져보세요.";
...
function scrollToCard(cardId, element) {
    ...
    if (targetCard) {
        ...
        targetCard.classList.add('ring-4', 'ring-primary/50', 'transition-all', 'duration-500');
        setTimeout(() => {
...
// 레벨 데이터 시스템 (0~20)
const levelSystem = [
    { lv: 0, en: "Insight Newbie", ko: "통찰 새싹", desc: "아직 아무것도 기록하지 않았지만, 생각을 남길 준비가 되었다.", next: "첫 기록 1개 작성" },
    { lv: 1, en: "Insight Starter", ko: "통찰 입문자", desc: "경험을 그냥 흘려보내지 않고, 처음으로 붙잡았다.", next: "기록 5개 작성" },
    { lv: 2, en: "Insight Explorer", ko: "통찰 탐색자", desc: "다양한 경험을 기록하며 자신의 관심사를 탐색하고 있다.", next: "기록 10개 + 수정 1회" },
    { lv: 3, en: "Insight Adventurer", ko: "통찰 모험가", desc: "기록이 일회성이 아니라는 걸 깨닫기 시작했다.", next: "기록 15개 + 주제 3개" },
    { lv: 4, en: "Insight Observer", ko: "통찰 관찰자", desc: "자신의 반응과 생각 패턴을 관찰하기 시작한다.", next: "기록 20개 + 보완 5회" },
    { lv: 5, en: "Insight Recorder", ko: "통찰 기록자", desc: "생각을 남기는 것이 자연스러운 습관이 되었다.", next: "재성찰 10회" },
    { lv: 6, en: "Insight Reflector", ko: "통찰 성찰자", desc: "같은 경험에서도 다른 생각이 나온다는 걸 이해한다.", next: "생각 확장 2회" },
    { lv: 7, en: "Insight Thinker", ko: "통찰 사유가", desc: "감상이 아닌 ‘왜 그렇게 느꼈는지’를 묻기 시작한다.", next: "생각 정리 10회" },
    { lv: 8, en: "Insight Analyzer", ko: "통찰 분석가", desc: "감정과 생각을 분리해 바라볼 수 있다.", next: "재방문 20회" },
    { lv: 9, en: "Insight Interpreter", ko: "통찰 해석자", desc: "과거의 자신을 현재의 언어로 다시 해석한다.", next: "기록 연결 5회" },
    { lv: 10, en: "Insight Synthesizer", ko: "통찰 종합자", desc: "흩어진 생각을 하나의 흐름으로 묶기 시작한다.", next: "장문 성찰 5회" },
    { lv: 11, en: "Insight Builder", ko: "통찰 구축자", desc: "생각을 쌓아 올려 하나의 구조를 만든다.", next: "일관성 유지 10회" },
    { lv: 12, en: "Insight Architect", ko: "통찰 설계자", desc: "자신만의 사고 프레임을 설계한다.", next: "행동 계획 5회" },
    { lv: 13, en: "Insight Strategist", ko: "통찰 전략가", desc: "생각을 삶의 전략으로 바꾸기 시작한다.", next: "큐레이션 20개" },
    { lv: 14, en: "Insight Curator", ko: "통찰 큐레이터", desc: "자신의 사유 아카이브를 선별하고 관리한다.", next: "연속 기록 10개" },
    { lv: 15, en: "Insight Scholar", ko: "통찰 연구자", desc: "반복된 사유를 통해 깊이를 확보한다.", next: "가치관 기록 5개" },
    { lv: 16, en: "Insight Philosopher", ko: "통찰 철학자", desc: "‘나는 무엇을 중요하게 여기는가’를 언어화한다.", next: "실행 후 회고 5회" },
    { lv: 17, en: "Insight Luminary", ko: "통찰 선구자", desc: "생각이 실제 삶의 변화를 만들어낸다.", next: "6개월 지속 + 재방문 50회" },
    { lv: 18, en: "Insight Authority", ko: "통찰 권위자", desc: "사유의 밀도와 지속성이 확실히 드러난다.", next: "재성찰 비율 30%" },
    { lv: 19, en: "Insight Master", ko: "통찰 마스터", desc: "기록 자체가 하나의 사고 훈련이 되었다.", next: "대표 성찰 3개" },
    { lv: 20, en: "Insight Sage", ko: "통찰 현자", desc: "경험을 통찰로 바꾸는 법을 완전히 체득했다.", next: "Max Level" }
];
...
function updateProfileUI() {
    // 안전장치: 레벨이 범위 밖이면 조정
    if(userStats.currentLevel >= levelSystem.length) userStats.currentLevel = levelSystem.length - 1;

    const lvData = levelSystem[userStats.currentLevel];
    
    document.getElementById("profile-level-badge").innerText = `Lv.${lvData.lv}`;
    document.getElementById("profile-title-display").innerText = `${lvData.en} (${lvData.ko})`;
    document.getElementById("profile-desc-display").innerText = `"${lvData.desc}"`;
    document.getElementById("profile-next-goal").innerText = `Next: ${lvData.next}`;

    let progress = Math.min((userStats.postCount / userStats.nextLevelGoal) * 100, 100);
